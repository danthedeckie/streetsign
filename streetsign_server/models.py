...
from streetsign_server.helpers.database import DBModel, now, safe_json_load
...
class Feed(DBModel):
    ...
    def dict_repr(self):
        ''' As it shows up in API/JSON views. '''
        return {
            'id': self.id,
            'name': self.name,
            'post_types': self.post_types_as_list(),
            'post_count': self.post_count(published=False, expired=True),
            'active_post_count': self.post_count(),
        }
